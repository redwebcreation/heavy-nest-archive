...
func RunSelfUpdateCommand(_ *cobra.Command, args []string) error {
	if dryRun {
		globals.Ansi.Warning("Dry running the command, nothing will be executed.")
	}
	...
	if usingGoRun {
		globals.Ansi.Warning("You're running this command using go run, you won't see any effects.")
		globals.Ansi.Warning("Please build the binary and then run it.")
		return nil
	}

	//goland:noinspection GoBoolExpressions
	if globals.Version == "(development)" {
		globals.Ansi.Warning("You're using the development build of Hez.")
		globals.Ansi.Warning("Please, specify a version to use when building the binary.")
		globals.Ansi.Warning("go build -ldflags=\"-X github.com/redwebcreation/hez2/globals.Version=$(git describe --tags)\"")
		return nil
	}
	...
	if !force && latestRelease.TagName == globals.Version {
		globals.Ansi.Success("You're already using Hez " + latestRelease.TagName + ".")
		return nil
	}
	...
	globals.Ansi.Printf("Downloading %s.\n", binary.BrowserDownloadUrl)
	...
	if dryRun {
		globals.Ansi.Success("Successfully updated Hez to the version " + latestRelease.TagName)
		return nil
	}
