...
import (
	...
	"github.com/redwebcreation/hez2/github"
...
func RunSelfUpdateCommand(_ *cobra.Command, args []string) error {
	...
	if globals.Version == "(development)" {
		...
		ansi.Warning("go build -ldflags=\"-X github.com/redwebcreation/hez2/globals.Version=$(git describe --tags)\"")
		return nil
	}

	releases, err := globals.Repository.Releases(github.ReleaseFilter{
		Draft:      draft,
	...
	if !force && latestRelease.TagName == globals.Version {
		ansi.Success("You're already using Hez " + latestRelease.TagName + ".")
		return nil
	}
